...
public class ShaderManager {
    ...
    private static class ShaderEffect {
        ...
        private PostChain createPostChain(String path) {
            ResourceLocation location = GallantTirelessAltruist.id(path);
            try {
        ...
        public boolean isValid() {
            return postChain != null && !((PostChainAccessor) postChain).getPasses().isEmpty();
        }

        public List<PostPass> getPasses() {
            return postChain != null ? ((PostChainAccessor) postChain).getPasses() : Collections.emptyList();
        }
    ...
    public static void onRegisterClientReloadListeners(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) resourceManager -> {
            // 延迟到主线程执行，避免线程安全问题
            MC.execute(ShaderManager::reloadAllShaders);
        });
    }
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_LEVEL) {
            ...
            // 处理所有启用的着色器
            EFFECTS.values().forEach(effect -> effect.process(event.getPartialTick().getRealtimeDeltaTicks()));
